import { IsNotEmpty, IsString, Matches, MinLength } from 'class-validator';

const COLOMBIAN_PHONE_PATTERN = /^(?:3\d{9}|573\d{9}|\+573\d{9})$/;

export class RequestPhonePasswordRecoveryDto {
  @IsString()
  @Matches(COLOMBIAN_PHONE_PATTERN, {
    message: 'Ingresa un celular colombiano válido',
  })
  phone!: string;
}

export class VerifyPhonePasswordRecoveryDto extends RequestPhonePasswordRecoveryDto {
  @IsString()
  @IsNotEmpty()
  challengeId!: string;

  @Matches(/^\d{6}$/, { message: 'El código debe contener seis números' })
  code!: string;
}

export class ResetPasswordByPhoneDto extends RequestPhonePasswordRecoveryDto {
  @IsString()
  @Matches(/^[a-f0-9]{64}$/, { message: 'Solicitud de recuperación inválida' })
  resetToken!: string;

  @IsNotEmpty({ message: 'La nueva contraseña es obligatoria' })
  @MinLength(8, {
    message: 'La contraseña debe tener al menos ocho caracteres',
  })
  @Matches(/[A-Z]/, {
    message: 'La contraseña debe contener una letra mayúscula',
  })
  @Matches(/[a-z]/, {
    message: 'La contraseña debe contener una letra minúscula',
  })
  @Matches(/[0-9]/, {
    message: 'La contraseña debe contener un número',
  })
  newPassword!: string;

  @IsNotEmpty({ message: 'Debes confirmar la contraseña' })
  confirmPassword!: string;
}
